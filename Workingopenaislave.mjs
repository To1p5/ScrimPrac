...
async function fetchReport(data) {
	...
	try {
		const url = 'https://hard-moonlightthu-9878.cocert56.workers.dev/api/message';
		apiMessage.innerText = 'Asking the AI guru...';
		...
		if (aiResponse && aiResponse.content) {
			renderReport(aiResponse.content);
		} else {
			throw new Error('Unexpected AI response format');
		}
	} catch (err) {
		console.error('Fetch report error:', err);
		loadingArea.innerText = `Error55: ${err.message}`;
	}
}
